...
function solution(board) {
    ...
    board.forEach((rowElement, row) => {
        rowElement.forEach((columnElement, column) => {
            let tmp = checkPlace(board, row, column);
            switch(tmp) {
                case 'vertex' :
...
function checkPlace(board, row, column) {
    ...
    else if (row === 0 && column === finalIndex && board[0][finalIndex] === 1) return 'vertex';
    else if (row === finalIndex && column === 0 && board[finalIndex][0] === 1) return 'vertex';
    else if (row === finalIndex && column === finalIndex && board[finalIndex][finalIndex] === 1) return 'vertex';

    else if (row === 0 && board[row][column] === 1) return 'side';
    else if (row === finalIndex && board[row][column] === 1) return 'side';
    else if (column === 0 && board[row][column] === 1) return 'side';
    else if (column === finalIndex && board[row][column] === 1) return 'side';

    else if (board[row][column] === 1) return 'inner';
    return 'none';
}
